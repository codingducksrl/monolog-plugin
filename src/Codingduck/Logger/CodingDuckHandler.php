...
class CodingDuckHandler extends AbstractProcessingHandler {
     ...
     */
    public function __construct(string $connectionString, array $credentials, $level = Logger::DEBUG, bool $bubble = true) {
        parent::__construct($level, $bubble);
        $this->connectionString = $connectionString;
        $this->connectionTimeout = (float)ini_get('default_socket_timeout');

        $this->caFile = $credentials["ca"];
        $this->certFile = $credentials["cert"];
        $this->pkFile = $credentials["key"];

        $this->setFormatter(new JsonFormatter(JsonFormatter::BATCH_MODE_NEWLINES, true, true));
    }
    ...
    protected function getTLSContext() {
        return stream_context_create([
            "ssl" => [
                "cafile" => $this->caFile,
                "local_cert" => $this->certFile,
                "local_pk" => $this->pkFile
            ]
        ]);
    }
    ...
    protected function pfsockopen() {
        return @stream_socket_client($this->connectionString, $this->errno, $this->errstr, $this->connectionTimeout, STREAM_CLIENT_PERSISTENT, $this->getTLSContext());
    }
    ...
    protected function fsockopen() {
        return @stream_socket_client($this->connectionString, $this->errno, $this->errstr, $this->connectionTimeout, STREAM_CLIENT_CONNECT, $this->getTLSContext());
    }
